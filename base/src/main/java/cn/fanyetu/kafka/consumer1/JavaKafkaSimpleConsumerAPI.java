...
public class JavaKafkaSimpleConsumerAPI {
	...
	private Map<KafkaTopicPartitionInfo, List<KafkaBrokerInfo>> replicaBrokers = new
			HashMap<KafkaTopicPartitionInfo, List<KafkaBrokerInfo>>();
	...
	public void run(long maxReads,
		...
		try {
			...
			long readOffSet = -1;
			while (true) {
				readOffSet = this.getLastOffSet(consumer, groupId, topic, partitionID, whichTime,
						clientName);
				if (readOffSet == -1) {
					// 当返回为-1的时候，表示异常信息
					if (times > this.maxRetryTimes) {
						throw new RuntimeException("Fetch the last offset of those group:" +
			...
			while (ever || maxReads > 0) {
				...
				if (response.hasError()) {
					...
					System.out.println("Error fetching data from the Broker:" + leaderBrokerHost +
							" Reason:" + code);
				...
				for (MessageAndOffset messageAndOffset : response.messageSet(topic, partitionID)) {
					...
					if (currentOffset < readOffSet) {
						System.out.println("Found and old offset:" + currentOffset + " Expection:"
								+ readOffSet);
						continue;
					}
	...
	private void validatePartitionMetadata(PartitionMetadata metadata) {
		if (metadata == null) {
			...
			throw new IllegalArgumentException("Can't find metadata for Topic and Partition. " +
					"Exiting!!");
		}
	...
	public PartitionMetadata findLeader(
		...
		for (KafkaBrokerInfo broker : brokers) {
			...
			try {
				// 1. 创建简单的消费者连接对象
				consumer = new SimpleConsumer(broker.brokerHost, broker.brokerPort, 100000, 64 *
						1024, "leaderLookUp");
						...
						for (PartitionMetadata part : metadata.partitionsMetadata()) {
							if (part.partitionId() == partitionID) {
								...
								if (returnMetadata != null) {
									KafkaTopicPartitionInfo topicPartitionInfo = new
											KafkaTopicPartitionInfo(topic, partitionID);
									List<KafkaBrokerInfo> brokerInfos = this.replicaBrokers.get
											(topicPartitionInfo);
									if (brokerInfos == null) {
									...
									for (BrokerEndPoint replica : returnMetadata.replicas()) {
										brokerInfos.add(new KafkaBrokerInfo(replica.host(),
												replica.port()));
									}
				...
				System.out.println("Error communicating with Broker [" + broker.brokerHost + "] to" +
						" find Leader for [" + topic + ", " + partitionID + "] Reason:" + e);
			} finally {
				if (consumer != null) {
					try {
						consumer.close();
	...
	public long getLastOffSet(SimpleConsumer consumer, String groupId,
		...
		long offset = this.getOffsetOfTopicAndPartition(consumer, groupId, clientName, topic,
				partitionID);
		if (offset > 0) {
			return offset;
		}
		...
		Map<TopicAndPartition, PartitionOffsetRequestInfo> requestInfoMap = new
	 ...
	 * @return
	 */
	public long getOffsetOfTopicAndPartition(SimpleConsumer consumer, String groupId, String
			clientName, String topic, int partitionID) {
		TopicAndPartition topicAndPartition = new TopicAndPartition(topic, partitionID);
		...
		if (returnOffsetMetadata != null && !returnOffsetMetadata.isEmpty()) {
			...
			if (offset.error() == ErrorMapping.NoError()) {
				...
				System.out.println("Error fetching data Offset Data the Topic and Partition. " +
						"Reason: " + offset.error());
			}
		}
	...
	private SimpleConsumer updateOffset(SimpleConsumer consumer, String topic, int partitionID,
										long readOffSet, String groupId, String clientName, int
												times) {
		// 构建请求对象
		Map<TopicAndPartition, OffsetAndMetadata> requestInfoMap = new HashMap<TopicAndPartition,
				OffsetAndMetadata>();
		TopicAndPartition topicAndPartition = new TopicAndPartition(topic, partitionID);
		// TODO ???
//		requestInfoMap.put(topicAndPartition, new OffsetAndMetadata(readOffSet, OffsetAndMetadata
//				.NoMetadata(), -1));
		kafka.javaapi.OffsetCommitRequest ocRequest = new OffsetCommitRequest(groupId,
				requestInfoMap, 0, clientName);
		// 提交修改偏移量的请求，并获取返回值
		...
		if (response.hasError()) {
			...
			if (code == ErrorMapping.LeaderNotAvailableCode()) {
				...
				consumer = this.createSimpleConsumer(metadata.leader().host(),
						metadata.leader().port(), clientName);
				// 重试
				consumer = updateOffset(consumer, topic, partitionID, readOffSet, groupId,
						clientName, times + 1);
			}
	 ...
	 * @return
	 */
	private SimpleConsumer createNewSimpleConsumer(SimpleConsumer consumer, String topic, int
			partitionID) {
		// 重新获取新的leader节点
		PartitionMetadata metadata = this.findNewLeaderMetadata(consumer.host(),
				topic, partitionID);
	 ...
	 * @return
	 */
	public static List<Integer> fetchTopicPartitionIDs(List<KafkaBrokerInfo> brokers, String
			topic, int soTimeout, int bufferSize, String clientId) {
		...
		for (KafkaBrokerInfo broker : brokers) {
			...
			try {
				// 构建简单消费者连接对象
				consumer = new SimpleConsumer(broker.brokerHost, broker.brokerPort, soTimeout,
						bufferSize, clientId);
